...
BOOL CAdoConnection::Connect(LPCTSTR strConnect, long lOptions)
{
	...
	try
	{
		...
		{
			// 连接数据库---------------------------------------------
			if (SUCCEEDED(m_pConnection->Open(strConnect, L"", L"", lOptions)))
			{	
				return TRUE;
			}
		}
...
BOOL CAdoRecordSet::GetValueString(CString& strValue, long lFieldIndex)
{
	...
	try
	{
		if (m_pRecordset == NULL || m_pRecordset->State == adStateClosed)
		{
			MessageBox(NULL,L"数据库可能已经断开,\r\n请重新连接、然后重试.",L"提示",MB_ICONINFORMATION);
			return FALSE;
		} 
...
BOOL CAdoRecordSet::GetCollect(_variant_t &index,long lFieldIndex)
{
	ASSERT(m_pRecordset != NULL);
	if (lFieldIndex < 0 || lFieldIndex >= GetFieldsCount())
	{
...
CString CAdoRecordSet::Format(_variant_t var)
{
	...
	case VT_I2://短整型
		...
		break;
	case VT_UI2://无符号短整型
		strValue.Format(L"%d", var.uiVal);
		break;
	case VT_VOID://
	case VT_INT://整型
	case VT_I4://整型
		strValue.Format(L"%d", var.intVal);
		break;
	case VT_I8://长整型
	...
	case VT_UINT://无符号整型
	case VT_UI4://无符号整型
		strValue.Format(L"%d", var.uintVal);
		break;
	case VT_UI8://无符号长整型
		strValue.Format(L"%d", var.ulVal);
		break;
	case VT_R4://浮点型
		strValue.Format(L"%.4f", var.fltVal);
		break;
	case VT_R8://双精度型
		strValue.Format(L"%.8f", var.dblVal);
		break;
	case VT_DECIMAL: //小数
		strValue.Format(L"%.4f", var.decVal);
		break;
	case VT_BOOL://布尔型
...
连接对象执行命令
====================*/
BOOL CAdoConnection::Execute(CString strSQL)
{
