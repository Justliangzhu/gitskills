// EarhBz.cpp: implementation of the EarhBz class.
//
//////////////////////////////////////////////////////////////////////
...
EarhBz_ROAD::EarhBz_ROAD()
{
	...
	EarhId = CreateLayer(L"EarhBz",L"CONTINUOUS");
}
...
AcDbObjectId EarhBz_ROAD::CreateLayer(ACHAR *LayerName, ACHAR *LineType)
{
...
Adesk::Boolean  EarhBz_ROAD::G_maketext(AcGiWorldDraw* pWd,AcGePoint3d& Pt, ACHAR* ctext,double ang,double texth ,int icolor,int mode=1)//在头文件中不能写成mode=1,应为mode
{   
...
Adesk::Boolean EarhBz_ROAD::subWorldDraw(AcGiWorldDraw* pWd)
{   
	AcGePoint3d Pt,tpt;
	JD_CENTER *pz;
	ACHAR str[50];
	...
	if (pz)
		...
		ads_printf(L"EarhBz:无法找到平面方案!\n");

	tpt.x = x_ori - 29;
	tpt.y = pORI_SCALE.Y_ORI - pORI_SCALE.DESHIGH - 0.5 * High;
	_tcscpy(str,L"地面高程");
	...
	if (m_Frame_Index >= 0)
	{
		...
		verts[0].x = x_ori;
		verts[0].y = pORI_SCALE.Y_ORI - pORI_SCALE.DESHIGH - pORI_SCALE.EARHIGH;
		verts[0].z = 0.0;
		verts[1].x = x_ori + pzLinep->LenOfLine / m_HScale;	//20190709	修改采用双轴比例尺
		verts[1].y = verts[0].y;
		verts[1].z = 0.0;
		pWd->subEntityTraits().setLayer(EarhId);//先设层
		...
		if(dmx)
		{
			...
			_stprintf(str,L"%0.2lf",dmx->DMXys[0].level);
			...
			for(int i=1; i<dmx->NUM-1;i++)
			{
				...
				_stprintf(str, L"%0.2lf", dmx->DMXys[i].level);
				G_maketext(pWd, Pt, str, 0.5*PI, texth, 0, 1);
				qcml = dmx->DMXys[i].lc;
				//}
			}
			Pt.x = cmltosx(dmx->DMXys[dmx->NUM-1].lc);
			_stprintf(str,L"%0.2lf",dmx->DMXys[dmx->NUM-1].level);
			G_maketext(pWd,Pt,str,0.5*PI,texth,0,1);
		}
	}
...
Acad::ErrorStatus EarhBz_ROAD::dwgInFields(AcDbDwgFiler* pFiler)
{
	assertWriteEnabled();
...
//
Acad::ErrorStatus EarhBz_ROAD::dwgOutFields(AcDbDwgFiler* pFiler) const
{
	assertReadEnabled();
...
Acad::ErrorStatus EarhBz_ROAD::dxfInFields(AcDbDxfFiler* pFiler)
{
	...
	if (!pFiler->atSubclassData(L"Earh")) {
		ads_printf(L"\n error 10010 类型有错 \n");
...
//
Acad::ErrorStatus EarhBz_ROAD::dxfOutFields(AcDbDxfFiler* pFiler) const
{
	assertReadEnabled();
	AcDbObject::dxfOutFields(pFiler);
	pFiler->writeItem(AcDb::kDxfSubclass, L"EarhBz");///////////"classname  Earh"//add///////////
...
void EarhBz_ROAD::set_HScale_ori()
{	
	High = pORI_SCALE.EARHIGH;
	x_ori = pORI_SCALE.X_ORI;
	y_ori = pORI_SCALE.Y_ORI - pORI_SCALE.DESHIGH - High*0.5;
...
double EarhBz_ROAD::cmltosx (double cml)
{
	double sx;
	sx = x_ori + (cml - pzLinep->DLArray[0].ELC) / m_HScale;	//20190709	修改采用双轴比例尺
	return sx;
}
Adesk::Boolean  EarhBz_ROAD::G_make2dline(AcGiWorldDraw* pWd,AcGePoint3dArray ptarray, int icolor)
{     
