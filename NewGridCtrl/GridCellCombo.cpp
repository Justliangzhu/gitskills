						   ...
						   UINT nFirstChar)
{
	...
	SetMe();
	// Add the strings
	for (int i = 0; i < Items.GetSize(); i++) //plh
		AddString(Items[i]);
...
CInPlaceListBox::CInPlaceListBox(CWnd* pParent, CRect& rect, DWORD dwStyle, UINT nID,
                           int nRow, int nColumn, 
						   CStringArray& Items, CString sInitText, 
						   UINT nFirstChar)
{
...
void CInPlaceListBox::PostNcDestroy() 
{
...
BEGIN_MESSAGE_MAP(CInPlaceListBox, CComboBox)
	//{{AFX_MSG_MAP(CInPlaceList)
	ON_WM_KILLFOCUS()
	ON_WM_KEYDOWN()
	ON_WM_KEYUP()
...
// Need to keep a lookout for Tabs, Esc and Returns.
void CInPlaceListBox::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if (nChar == VK_ESCAPE) 
		SetWindowText(m_sInitText);	// restore previous text
...
BOOL CGridCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	//m_pEditWnd = new CInPlaceListBox(GetGrid(), rect, GetStyle(), nID, nRow, nCol, 
...
CWnd* CGridCellCombo::GetEditWnd() const
{
	if (m_pEditWnd && (m_pEditWnd->GetStyle() & CBS_DROPDOWNLIST) != CBS_DROPDOWNLIST )
		return &(((CInPlaceListBox*)m_pEditWnd)->m_edit);
...
//	AfxMessageBox(L"1");
//    m_Strings.RemoveAll();
//	m_Strings.Add(ar[ar.GetSize()-1]);//plh
