...
BOOL CGridCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	int nRowsScrolled = m_nRowsPerWheelNotch * zDelta / 20;
	...
	if (m_nRowsPerWheelNotch == -1)
	{
		...
		{
			rect.top = GetFixedRowHeight();
			int offset = (int)(-0.5 * rect.Height());
			int pos = max(0, scrollPos + offset);
			SetScrollPos32(SB_VERT, pos);
			rect.top = GetFixedRowHeight();
			InvalidateRect(rect);
		...
		{
			rect.top = GetFixedRowHeight();
			scrollPos = int(min(m_nVScrollMax, scrollPos + 0.5 * rect.Height()));
			SetScrollPos32(SB_VERT, scrollPos);
			rect.top = GetFixedRowHeight();
			InvalidateRect(rect);
		}
...
// 				}
	...
	int col=GetFocusCell().col;
	int row=GetFocusCell().row;
	SetItemText(row,col,InsertRail);
	Invalidate();
...
//我的函数
BOOL CGridCtrl::SetColumnNoEditable(int nCol)
{
	ASSERT(nCol>=0 &&nCol<m_nCols);
	if(nCol<0 || nCol>=m_nCols)
		return FALSE;
...
BOOL CGridCtrl::GetColumnNoEditable(int nCol)
{
	ASSERT(nCol>=0 &&nCol<m_nCols);
	if(nCol<0 || nCol>=m_nCols)
		return FALSE;
	...
	int nNumRows=GetRowCount();
	for(int nRow=nFixedRows;nRow<nNumRows;nRow++)
	{
		CGridCellBase  *pCell=GetCell(nRow,nCol);
		if(pCell->IsReadOnly())
			return true;
...
//*****************************************************************
BOOL CGridCtrl::AddComboColumn(int iCol, CStringArray& comStr)
{
	...
	//{
	//	SetCellType(row, iCol, RUNTIME_CLASS(CGridCell));
	//	SetItemState(row, iCol, GetItemState(row, iCol) & ~GVIS_READONLY);
	//	Invalidate();
	//	if (!SetCellType(row, iCol, RUNTIME_CLASS(CGridCellCombo)))
	//		continue;

	//	CGridCellCombo* pCell = (CGridCellCombo*)GetCell(row, iCol);
	//	pCell->SetOptions(comStr);
