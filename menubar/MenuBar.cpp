...
static const ACHAR _szMoreWindows[] = L"&More windows...";
...
BEGIN_MESSAGE_MAP(CMenuBar, CControlBar)
	...
	//	ON_COMMAND(ID_MNU_CWDIR, OnMnuCwdir)
...
BOOL CMenuBar::LoadMenuBar(UINT nIDResource)
{
	...
	//sunan:要不使原AutoCAD的菜单消失应注释掉一下代码
	...
	//	m_menu.Detach();
	//ppp	m_hMenu = ::LoadMenu(hInstance3, MAKEINTRESOURCE(nIDResource));
	//sunan注释
...
void CMenuBar::OnInitMenuPopup(BCMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
	...
	for (iWin = 1; iWin <= 10; iWin++, nID++)
	{
		...
		{
			//已修改
			menu.InsertMenu(iPos++, MF_BYPOSITION | MF_SEPARATOR, 3, NULL);
			bAddSeperator = FALSE;
		}
		...
		if (iWin < 10)
		{
			...
			sMenuItem.Format(L"&%d %s", iWin, (LPCTSTR)sWinName);
			ACHAR TEMP[200];
			_tcscpy(TEMP,sMenuItem);
			menu.InsertMenu(iPos, MF_BYPOSITION, nID, TEMP);
			if (hwnd == hwndActive)
				menu.CheckMenuItem(iPos, MF_BYPOSITION | MF_CHECKED);
		}
		else
		{
			menu.InsertMenu(iPos, MF_BYPOSITION, nID,  L"&More windows...");
		}
		iPos++;
	}
...
LRESULT CMainFrameHook::WindowProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITMENUPOPUP:
		if (!HIWORD(lParam) && (HMENU)wParam == m_pMenuBar->m_hWindowMenu)
			m_pMenuBar->OnInitMenuPopup((BCMenu*)CMenu::FromHandle((HMENU)wParam),
...
void CMenuIcon::OnActivateChildWnd()
{
	...
	m_hDocIcon = (HICON)::GetClassLong(hWndChild, GCLP_HICONSM);
